	...
	if (cot_plugin_active('autoalias2'))
	{
		...
		do
		{
			...
			if (!$cfg['plugin']['autoalias2']['prepend_id']
				&& $db->query("SELECT COUNT(*) FROM $db_folio WHERE item_alias = '".$ritem['item_alias']."' AND item_id != $id")->fetchColumn() > 0)
			{
				$duplicate = true;
			}
			else
			{
				$db->update($db_folio, array('item_alias' => $ritem['item_alias']), "item_id = $id");
				$duplicate = false;
			}
		}
